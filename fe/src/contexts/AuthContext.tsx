...
interface AuthContextType {
  ...
  isLoading: boolean;
  /** Đang đăng xuất và SẮP điều hướng đi — giữ màn chờ, đừng render
...
export const AuthProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  const logout = useCallback(async (dichDen?: string) => {
    try {
      ...
      // Bật cờ TRƯỚC khi dọn state: dọn xong là React vẽ lại ngay, trong khi
      // việc rời trang còn chờ `await fetch(...)` — khoảng hở đó đủ để
      // ProtectedRoute đá sang /login và màn đăng nhập cục bộ loé lên (đăng
      // xuất nháy hai lần). Chỉ bật khi thật sự sắp điều hướng đi.
      const identityUrlSom = import.meta.env.VITE_IDENTITY_URL as string | undefined;
      if (dichDen ?? identityUrlSom) setIsLoggingOut(true);

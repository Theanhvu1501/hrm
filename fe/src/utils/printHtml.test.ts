...
import { buildPrintableDocument, printHtml } from "./printHtml";
...
describe("buildPrintableDocument", () => {
  it("bọc HTML đã ghép dữ liệu thành 1 document hoàn chỉnh có tiêu đề mặc định", () => {
    ...
    expect(out).toContain("<title>In</title>");
...
describe("printHtml — iframe in phải được sandbox (review Critical 1 + High)", () => {
  afterEach(() => {

...
> **Bẫy đã kiểm chứng — đừng import tĩnh.** `IDENTITY_URL` là const cấp module,
> đọc `import.meta.env.VITE_IDENTITY_URL` **một lần lúc import**. Vitest chạy
> mode `test` và repo không có `.env.test`, nên giá trị đó là `undefined` khi
> test import tĩnh → cả ba hàm rơi vào nhánh "chưa cấu hình" và test hỏng.
>
> Dùng đúng khuôn đã có sẵn cho chính const này ở `identitySession.test.ts:5,41-73`:
> `vi.resetModules()` trong `beforeEach`, rồi `vi.stubEnv('VITE_IDENTITY_URL', …)`
> và `await import('./identitySession')` **trong thân từng test**. Bọc thành một
> helper cho gọn:
>
> ```ts
...
> ```
>
> **Không** sửa `vitest.config.ts` để đặt env toàn cục — đã thử, nó lật kỳ vọng
> "chưa cấu hình" của `identityRedirect` và `ssoHandoff`.

Toàn bộ giá trị và assertion dưới đây giữ nguyên, chỉ đổi cách lấy hàm ra:

```ts
import { describe, it, expect, vi, beforeEach, afterEach } from 'vitest';

const IDENTITY = 'http://localhost:3020';

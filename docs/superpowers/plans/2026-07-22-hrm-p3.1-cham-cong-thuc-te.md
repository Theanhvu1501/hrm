...
describe('maLoiChamCong', () => {
  it('lấy được code từ ApiError bọc lỗi axios', () => {
    const loiAxios = {
      response: { data: { code: 'THIET_BI_CHO_DUYET', message: 'Chờ duyệt' } },
    };
  ...
  it('lấy được code từ lỗi axios trần', () => {
    expect(
      maLoiChamCong({ response: { data: { code: 'THIET_BI_BI_THU_HOI' } } }),
...
export function maLoiChamCong(err: unknown): string | undefined {
  ...
  const data = (goc as { response?: { data?: { code?: string } } })?.response
    ?.data;
  return data?.code;
}
